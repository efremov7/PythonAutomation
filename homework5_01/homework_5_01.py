...
def search_cars(car_data, search_criteria):
    ...
    filtered = [
        ...
        if specs[1] >= min_year          # рік випуску
        and specs[2] >= min_engine       # об'єм двигуна
        and specs[4] <= max_price        # ціна
    ]

    # Сортування за ціною (індекс 5 після додавання бренду на початок)
    filtered.sort(key=lambda car: car[5])

    return filtered[:5]


results = search_cars(car_data, search_criteria)

print(f"Критерії пошуку: рік >= {search_criteria[0]}, "
      f"об'єм двигуна >= {search_criteria[1]}л, "
      f"ціна <= ${search_criteria[2]:,}\n")

print(f"{'Бренд':<15} {'Колір':<10} {'Рік':<6} {'Двигун':<8} {'Тип':<12} {'Ціна':>10}")
print("-" * 65)

for car in results:
    brand, color, year, engine, car_type, price = car
    print(f"{brand:<15} {color:<10} {year:<6} {engine:<8} {car_type:<12} ${price:>9,}")